...
This cost is significant since our target use case will be calling these methods millions of times. However, some preliminary benchmarks have shown that expected performance is in the same order of magnitude. When we take `Linie` performance as a reference, the `Genie` performance on matrix inversion (a demanding operation) is about 0.09 times slower when benchmarked in a tight loop of a million iterations (29.19s and 32s for `Linie` and `Genie` respectively in [LINQPad](https://www.linqpad.net/)). This compared `Linie.Matrix4x4` (which uses `double`) to the `Genie.Matrix4x4<double>` generic implementation. Using a `Genie.Matrix4x4<float>` is a little bit slower (33s) but still well within the same order of magnitude.
